#!/usr/bin/env python3
"""
AI Russia API auditor, public reproducible edition.
...
DEFAULT_USER_AGENT = (
    ...
    "ai-russia-api-audit/public-1.0"
)
...
class ApiClient:
    def __init__(
        ...
        *,
        ...
        self.max_backoff = max(self.backoff, max_backoff)
        ...
        for attempt in range(self.retries + 1):
            ...
            try:
                ...
                if attempt >= self.retries:
                    raise
                wait_for = min(self.max_backoff, self.backoff * (2**attempt))
                logging.warning(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--delay", type=float, default=0.6, help="Delay between live HTTP requests")
    parser.add_argument("--retries", type=int, default=8, help="Retries for HTTP 429/5xx and network errors")
    parser.add_argument("--backoff", type=float, default=2.0, help="Initial exponential retry delay in seconds")
    parser.add_argument("--max-backoff", type=float, default=90.0, help="Maximum retry delay in seconds")
